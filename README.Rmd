---
title: "Vaccine effectiveness (VE) estimation using the screening method, upgraded with splines"
...
knitr::opts_chunk$set(echo = TRUE, fig.width = 12, dpi = 300, dev = "ragg_png")
library(data.table)
library(ggplot2)
theme_set(theme_bw())
set.seed(1)
...
A spline regresszióhoz segédanyagként elérhető a [magyar nyelvű jegyzetem](https://ferenci-tamas.github.io/simitas-spline/) a témában.

## Manuscript

The paper is available at [https://www.mdpi.com/2076-393X/10/11/1824](https://www.mdpi.com/2076-393X/10/11/1824).
...
This comes as no surprise as $VE$ (more precisely $1-VE$) itself is an odds ratio, since $VE = 1 - AR_V/AR_U$, where $AR$ stands for attack rate (number of infected divided by the size of the population); $V$ subscript indicates vaccinated, $U$ denotes unvaccinated subjects. By rearranging the terms, we arrive to the following expression:
...
We will use `r version$version.string` with packages `mgcv` (version `r packageVersion("mgcv")`), `data.table` (version `r packageVersion("data.table")`), `ggplot2` (version `r packageVersion("ggplot2")`).
...
We download and reshape the vaccine uptake data (in contrast to the previous, artifical data, these will be the actual values from the ECDC):
...
Finally, we simulate the number of people in the different categories (unvaccinated, partially vaccinated, fully vaccinated) based on the ground truth VE, and then convert the database to a case-based one (noting that the vaccine's effect's onset is assumed to take two weeks):
...
We create a plot to check, only visually for the time being, how well the estimated VEs match the ground truth (dotted line shpws the estimated, unsmoothed value, solid line indicates the estimated smoothed value, dashed is the ground truth):
...
- Validating the GAM (e.g., $k$ value, or different basis function; or `gam.check()`)
- Trying adaptive basis (i.e., `bs = "ad"`); seems to have huge computational burden.
